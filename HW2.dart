...
class Grid {
  ...
  Grid(int n) : grid = [] {
    // : grid is an initializer. (executes before body)
    this.n = n;
  ...
  void setNeighbors(int x, int y, int dist, int value) {
    // up
    if (x - dist >= 0 && value > grid[x - dist][y]) {
      // x - dist: avoid index out of range
      grid[x - dist][y] = value;
    }
    // up-right (diagonal)
  ...
  void printGrid() {
    for (int i = 0; i < n; i++) {
      String rowStr =
          "[" +
          grid[i].join(",  ") +
          "]"; // join allows to convert List to String with a separator
      print(rowStr);
    }
  }
}
